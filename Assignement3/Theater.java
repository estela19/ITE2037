...
public class Theater {
    ...
    public synchronized void registTicket(Movie movie, Member member, String seat){
        member.addTicket(new Ticket(movie, seat, totalticket));
        totalticket += 1;
        movie.newTicket(seat);
    }
    ...
    private void ReservationMovie(Movie movie, Member member){
        try {
            ...
            if(member.IsManager() == 1){
                System.out.printf(movie.getName() + " 영화의 좌석 예매 점유율: %.2f%%\n", (float)movie.getTicketnum()/Movie.MAX_TICKET_COUNT * 100);
            ...
            if (movie.getTicketnum() == Movie.MAX_TICKET_COUNT){
                ...
                long waitTime = Long.parseLong(scan.nextLine());

                movie.addWait(this, member, waitTime);
            }
            else{
                ...
                if (!(movie.getSeat(tmpseat))) {
                    registTicket(movie, member, seat);
    ...
    private void CancelReservation(User user){
        ...
        for (int i = 0; i < tickets.size(); ++i){
            System.out.printf("%d. %s%n", i+1, tickets.elementAt(i));
        }
        ...
        Ticket sel = tickets.elementAt(selected-1);
        Movie movie = sel.getMovie();
        user.removeTicket(sel);
        movie.sellTicket(sel.getSeat());
    }
